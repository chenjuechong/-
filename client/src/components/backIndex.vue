    ...
    <div class="backlogin">
       <div class="header">
         <div class="search_box" :class="{search_box_fouce:search_box_fouce}">
             ...
             <input @focus="focusFn" @blur="blurFn" type="text" name="" id="" value="" placeholder="Search some.."/>
         </div>
         <div class="handler">
            ...
            <img src="../assets/images/teacherimg01.png" alt="Oh NO" />
         </div> 
       </div>
      <!-- 侧面导航-->
      <div class="sldenav_box">
...
export default {
   ...
   methods: {
      focusFn(){ // 搜索框后去焦点，添加class
         this.search_box_focus = true;
      },
      blurFn(){ // 搜索框失去焦点 去掉class
         this.search_box_focus = false;
      },
      toggleSlide(){ // 这个用来显示和隐藏头像旁的下拉框
   ...
   watch:{ // 监控路径变化 当路径发送变化的时候，改编面包屑导航显示
      $route: {
         handler: function(val, oldVal) {
            var path = val.path;
            this.pageTitle = pageTitleObj[path.substr(path.lastIndexOf("/")+1)]
         }
      }
   }
}
