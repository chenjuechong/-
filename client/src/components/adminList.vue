...
<template>
    <div class="adminList main">
        <div class="input_box">
            <input v-model="Admin.name" type="text" class="myinput" placeholder="用户名" />
            <input v-model="Admin.phone" type="text" class="myinput" placeholder="手机号"/>
...
export default {
   ...
   methods: {
       ...
       addAdmin(){ //添加用户
            ...
            this.$reqs.post('/users/add',this.Admin).then((result)=>{
                //成功
                this.getAdminList();
                this.emptyAdmin();
            }).catch(function(error){
       ...
       saveEditAdmin(){
           ...
           this.$reqs.post('/users/update', this.Admin).then((result)=>{
               //成功
               this.gopage(this.pageInfo.currrent);
       ...
       deleteAdmin(item){
           this.$reqs.post('/users/delete',item).then((result)=>{
               //成功
               console.log(result)
               this.gopage(this.pageInfo.currrent);
               this.emptyAdmin();
